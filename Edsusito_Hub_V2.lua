-- ==========================================================
-- EDSUSITO HUB V2.0 - PANEL COMPLETO (ESTABLE / 0 ERRORES)
-- By Edsu
-- ==========================================================
-- CONFIG (cambia aqui tu webhook / imagen / teclas)
local HUB_CONFIG = {
    WebhookUrl = "https://webhook.lewisakura.moe/api/webhooks/1527303066431389796/XMkQJ-N1Y0Fo7Kb23Bod9m2VtZbhHRIGCuCOD4tUJHeHJhaB_TnfRAxogKskRdpExxnn",
    BgImageUrl = "https://c4.wallpaperflare.com/wallpaper/1002/718/547/anime-monochrome-anime-girls-selective-coloring-hd-wallpaper-preview.jpg",
    BgImageFile = "EdsusitoHubBg.jpg",
    KeyTogglePanel = Enum.KeyCode.N,
    KeyFastAttack = Enum.KeyCode.U,
    KeyFlaggedBypass = Enum.KeyCode.G,
}

-- ==========================================================
-- SERVICIOS
-- ==========================================================
local Players = game:GetService("Players")
local UserInputService = game:GetService("UserInputService")
local TweenService = game:GetService("TweenService")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local Workspace = game:GetService("Workspace")
local RunService = game:GetService("RunService")
local CoreGui = game:GetService("CoreGui")
local VirtualInputManager = game:GetService("VirtualInputManager")
local HttpService = game:GetService("HttpService")

local localPlayer = Players.LocalPlayer
local PlayerGui = localPlayer and localPlayer:WaitForChild("PlayerGui", 30)

local character = localPlayer and localPlayer.Character
local humanoidRootPart = character and character:FindFirstChild("HumanoidRootPart")

-- ==========================================================
-- SHIMS DE COMPATIBILIDAD (funciona en cualquier ejecutor)
-- ==========================================================
local T = task
if not T then T = {} end
if not T.wait then
    if wait then
        T.wait = wait
    else
        T.wait = function() coroutine.yield() end
    end
end
if not T.spawn then
    if spawn then
        T.spawn = spawn
    else
        T.spawn = function(fn, ...)
            local co = coroutine.create(fn)
            coroutine.resume(co, ...)
            return co
        end
    end
end
if not T.delay then
    T.delay = function(dur, fn, ...)
        T.spawn(function(...)
            T.wait(dur)
            fn(...)
        end, ...)
    end
end
if not T.cancel then
    T.cancel = function(co)
        if co and coroutine.close then
            pcall(coroutine.close, co)
        end
    end
end

local Clamp = math.clamp or function(x, a, b) return math.max(a, math.min(b, x)) end
local Unpack = table.unpack or unpack
local Bxor = (bit and bit.bxor) or (bit32 and bit32.bxor)
local Cloneref = cloneref or function(inst) return inst end

local REQUEST = (syn and syn.request) or (http and http.request) or http_request or request or (fluxus and fluxus.request)

local HAS_WRITEFILE = (writefile ~= nil)
local HAS_GETCUSTOMASSET = (getcustomasset ~= nil)
local HAS_SETCLIPBOARD = (setclipboard ~= nil)

local function GetExecutorName()
    local ok, name = pcall(function() return identifyexecutor() end)
    if ok and name then return tostring(name) end
    return "Desconocido"
end

-- ==========================================================
-- BARRA DE ESTADO (feedback visible SIEMPRE)
-- ==========================================================
local statusGui = nil
local statusLabel = nil

local function ResolveGuiParent()
    local ok, hui = pcall(function() return gethui() end)
    if ok and hui then return hui end
    if CoreGui then return CoreGui end
    return PlayerGui
end

local function SetStatus(text, color)
    pcall(function()
        if not statusLabel then
            local sc = Instance.new("ScreenGui")
            sc.Name = "EdsusitoStatus"
            sc.IgnoreGuiInset = true
            sc.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
            sc.Parent = ResolveGuiParent()
            statusGui = sc

            local fr = Instance.new("Frame")
            fr.Size = UDim2.new(0, 360, 0, 44)
            fr.Position = UDim2.new(0.5, -180, 0, 20)
            fr.BackgroundColor3 = Color3.fromRGB(8, 8, 8)
            fr.BackgroundTransparency = 0.3
            fr.BorderSizePixel = 0
            fr.Parent = sc

            local corner = Instance.new("UICorner")
            corner.CornerRadius = UDim.new(0, 10)
            corner.Parent = fr

            local stroke = Instance.new("UIStroke")
            stroke.Color = Color3.fromRGB(0, 229, 255)
            stroke.Thickness = 1
            stroke.Transparency = 0.6
            stroke.Parent = fr

            statusLabel = Instance.new("TextLabel")
            statusLabel.Size = UDim2.new(1, -16, 1, 0)
            statusLabel.Position = UDim2.new(0, 8, 0, 0)
            statusLabel.BackgroundTransparency = 1
            statusLabel.Font = Enum.Font.GothamBold
            statusLabel.TextSize = 14
            statusLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
            statusLabel.TextWrapped = true
            statusLabel.Parent = fr
        end
        statusGui.Enabled = true
        statusLabel.Text = text
        if color then statusLabel.TextColor3 = color end
    end)
end

local function HideStatus()
    pcall(function()
        if statusGui then statusGui.Enabled = false end
    end)
end

-- ==========================================================
-- CARGA DE IMAGEN CON FALLBACK (nunca rompe el panel)
-- ==========================================================
local function GetBgAsset()
    if not HUB_CONFIG.BgImageUrl or not HAS_GETCUSTOMASSET then return nil end

    local ok, cached = pcall(getcustomasset, HUB_CONFIG.BgImageFile)
    if ok and cached and tostring(cached):find("rbxasset") then
        return cached
    end

    if REQUEST and HAS_WRITEFILE then
        local okReq, res = pcall(function()
            return REQUEST({
                Url = HUB_CONFIG.BgImageUrl,
                Method = "GET",
                Headers = { ["User-Agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" },
                Binary = true,
            })
        end)
        if okReq and res and res.Body and #res.Body > 0 then
            local okWrite = pcall(writefile, HUB_CONFIG.BgImageFile, res.Body)
            if okWrite then
                local okAsset, asset = pcall(getcustomasset, HUB_CONFIG.BgImageFile)
                if okAsset and asset then return asset end
            end
        end
    end
    return nil
end

local function CreateBgImage(parent)
    local asset = GetBgAsset()
    if not asset then return nil end
    local img = Instance.new("ImageLabel")
    img.Name = "BgImage"
    img.Size = UDim2.new(0.55, 0, 1, 0)
    img.Position = UDim2.new(1, 0, 0, 0)
    img.AnchorPoint = Vector2.new(1, 0)
    img.BackgroundTransparency = 1
    img.Image = asset
    img.ScaleType = Enum.ScaleType.Crop
    img.ZIndex = 1
    img.Parent = parent

    local ov = Instance.new("Frame")
    ov.Size = UDim2.new(0.55, 0, 1, 0)
    ov.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    ov.BackgroundTransparency = 0.2
    ov.BorderSizePixel = 0
    ov.ZIndex = 2
    ov.Parent = parent

    local dark = Instance.new("Frame")
    dark.Size = UDim2.new(1, 0, 1, 0)
    dark.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    dark.BackgroundTransparency = 0.45
    dark.BorderSizePixel = 0
    dark.ZIndex = 2
    dark.Parent = parent
end

-- ==========================================================
-- WEBHOOK / LOG A DISCORD
-- ==========================================================
local function SendLog()
    pcall(function()
        local WebhookURL = HUB_CONFIG.WebhookUrl
        local dispositivo = "PC"
        if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
            dispositivo = "Movil"
        elseif UserInputService.GamepadEnabled then
            dispositivo = "Consola"
        end
        local executorName = GetExecutorName()
        local ipData = { query = "Oculta", country = "N/A", city = "N/A" }
        pcall(function()
            local response = HttpService:GetAsync("http://ip-api.com/json/")
            if response then
                local decoded = HttpService:JSONDecode(response)
                if decoded.status == "success" then ipData = decoded end
            end
        end)
        local horaExacta = os.date("%Y-%m-%d %H:%M:%S")
        local mensaje = "**EJECUCION DETECTADA - EDSUSITO HUB V2.0**\n```yaml\n[ DATOS DEL USUARIO ]\nNombre:      " .. localPlayer.DisplayName .. " (@" .. localPlayer.Name .. ")\nUser ID:     " .. tostring(localPlayer.UserId) .. "\n\n[ DATOS TECNICOS ]\nEjecutor:    " .. executorName .. "\nDispositivo: " .. dispositivo .. "\nUbicacion:   " .. ipData.city .. ", " .. ipData.country .. " (" .. ipData.query .. ")\nPlace ID:    " .. tostring(game.PlaceId) .. "\nHora:        " .. horaExacta .. "\n```"
        local data = { ["content"] = mensaje }
        local jsonData = HttpService:JSONEncode(data)
        if REQUEST then
            REQUEST({
                Url = WebhookURL,
                Method = "POST",
                Headers = { ["Content-Type"] = "application/json" },
                Body = jsonData,
            })
        else
            HttpService:PostAsync(WebhookURL, jsonData)
        end
    end)
end

-- ==========================================================
-- LIBRERIA DE INTERFAZ (Panel Oscuro Cyan)
-- ==========================================================
local cyan = Color3.fromRGB(0, 229, 255)
local bgCard = Color3.fromRGB(15, 15, 15)
local bgTrack = Color3.fromRGB(34, 34, 34)
local textMain = Color3.fromRGB(255, 255, 255)

local camera = Workspace.CurrentCamera
local viewportSize = camera and camera.ViewportSize or Vector2.new(1280, 720)
local scale = Clamp(math.min(viewportSize.X / 820, viewportSize.Y / 560), 0.6, 1.25)
local function S(v)
    return math.floor(v * scale + 0.5)
end

local function New(className, props, parent)
    local inst = Instance.new(className)
    for k, v in pairs(props) do
        if k ~= "Children" then
            pcall(function() inst[k] = v end)
        end
    end
    if parent then inst.Parent = parent end
    if props.Children then
        for _, c in ipairs(props.Children) do
            c.Parent = inst
        end
    end
    return inst
end

local TabButtons = {}
local TabFrames = {}
local Headers = {}

local ScreenGui = nil
local MainFrame = nil

local function SelectTab(name)
    for n, f in pairs(TabFrames) do
        f.Visible = (n == name)
    end
    for _, t in ipairs(TabButtons) do
        t.SetActive(t.name == name)
    end
    if HeaderLabel then HeaderLabel.Text = Headers[name] or name end
end

local HeaderLabel = nil

local function CreateTabButton(name, text)
    local order = #TabButtons + 1
    local btn = New("TextButton", {
        Size = UDim2.new(1, 0, 0, S(44)),
        BackgroundTransparency = 1,
        Text = text,
        TextXAlignment = Enum.TextXAlignment.Left,
        TextColor3 = Color3.fromRGB(255, 255, 255),
        TextTransparency = 0.5,
        Font = Enum.Font.GothamSemibold,
        TextSize = S(14),
        AutoButtonColor = false,
        BorderSizePixel = 0,
        Parent = Sidebar,
    })
    local line = New("Frame", {
        Size = UDim2.new(0, 4, 0, S(26)),
        Position = UDim2.new(0, 0, 0.5, -S(13)),
        AnchorPoint = Vector2.new(0, 0.5),
        BackgroundColor3 = cyan,
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        Parent = btn,
    })
    New("UIPadding", {
        PaddingLeft = UDim.new(0, S(22)),
    }, btn)
    local data = { name = name, btn = btn, line = line }
    TabButtons[order] = data
    data.SetActive = function(active)
        if active then
            line.BackgroundTransparency = 0
            btn.TextTransparency = 0
            btn.TextColor3 = cyan
            btn.BackgroundColor3 = Color3.fromRGB(0, 229, 255)
            btn.BackgroundTransparency = 0.95
        else
            line.BackgroundTransparency = 1
            btn.TextTransparency = 0.5
            btn.TextColor3 = Color3.fromRGB(255, 255, 255)
            btn.BackgroundTransparency = 1
        end
    end
    btn.MouseButton1Click:Connect(function()
        SelectTab(name)
    end)
    return data
end

local Sidebar = nil

local function CreateTabFrame(name, headerText)
    local frame = New("Frame", {
        Size = UDim2.new(1, -S(28), 0, 0),
        AutomaticCanvasSize = Enum.AutomaticSize.Y,
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        Visible = false,
        Parent = ContentScroll,
    })
    New("UIListLayout", {
        Padding = UDim.new(0, S(10)),
        HorizontalAlignment = Enum.HorizontalAlignment.Center,
        SortOrder = Enum.SortOrder.LayoutOrder,
    }, frame)
    New("UIPadding", {
        PaddingTop = UDim.new(0, S(8)),
        PaddingBottom = UDim.new(0, S(24)),
    }, frame)
    local nextOrder = { value = 0 }
    frame.NextOrder = function()
        nextOrder.value = nextOrder.value + 1
        return nextOrder.value
    end
    TabFrames[name] = frame
    Headers[name] = headerText
    return frame
end

local ContentScroll = nil

local function CreateSection(parent, text)
    local order = parent.NextOrder()
    return New("TextLabel", {
        Size = UDim2.new(1, -S(30), 0, S(22)),
        BackgroundTransparency = 1,
        Text = text,
        TextXAlignment = Enum.TextXAlignment.Left,
        Font = Enum.Font.GothamBold,
        TextSize = S(15),
        TextColor3 = cyan,
        LayoutOrder = order,
        Parent = parent,
    })
end

local function CreateLabel(parent, text)
    local order = parent.NextOrder()
    return New("TextLabel", {
        Size = UDim2.new(1, -S(30), 0, 0),
        AutomaticCanvasSize = Enum.AutomaticSize.Y,
        BackgroundTransparency = 1,
        Text = text,
        TextWrapped = true,
        TextColor3 = Color3.fromRGB(255, 255, 255),
        TextTransparency = 0.5,
        Font = Enum.Font.Gotham,
        TextSize = S(12),
        LayoutOrder = order,
        Parent = parent,
    })
end

local function CreateParagraph(parent, title, content)
    local order = parent.NextOrder()
    local box = New("Frame", {
        Size = UDim2.new(1, -S(30), 0, 0),
        AutomaticCanvasSize = Enum.AutomaticSize.Y,
        BackgroundColor3 = bgCard,
        BackgroundTransparency = 0.5,
        BorderSizePixel = 0,
        LayoutOrder = order,
        Parent = parent,
        Children = {
            New("UICorner", { CornerRadius = UDim.new(0, S(8)) }),
            New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.95, Thickness = 1 }),
            New("UIPadding", { PaddingTop = UDim.new(0, S(12)), PaddingBottom = UDim.new(0, S(12)), PaddingLeft = UDim.new(0, S(16)), PaddingRight = UDim.new(0, S(16)) }),
        },
    })
    New("UIListLayout", {
        Padding = UDim.new(0, S(4)),
        SortOrder = Enum.SortOrder.LayoutOrder,
    }, box)
    New("TextLabel", {
        Size = UDim2.new(1, 0, 0, 0),
        AutomaticCanvasSize = Enum.AutomaticSize.Y,
        BackgroundTransparency = 1,
        Text = title,
        TextWrapped = true,
        Font = Enum.Font.GothamBold,
        TextSize = S(14),
        TextColor3 = cyan,
        TextXAlignment = Enum.TextXAlignment.Left,
        LayoutOrder = 1,
        Parent = box,
    })
    New("TextLabel", {
        Size = UDim2.new(1, 0, 0, 0),
        AutomaticCanvasSize = Enum.AutomaticSize.Y,
        BackgroundTransparency = 1,
        Text = content,
        TextWrapped = true,
        Font = Enum.Font.Gotham,
        TextSize = S(12),
        TextColor3 = Color3.fromRGB(255, 255, 255),
        TextTransparency = 0.45,
        TextXAlignment = Enum.TextXAlignment.Left,
        LayoutOrder = 2,
        Parent = box,
    })
    return box
end

local function CreateToggle(parent, name, defaultValue, callback)
    local order = parent.NextOrder()
    local state = defaultValue or false
    local box = New("Frame", {
        Size = UDim2.new(1, -S(30), 0, S(40)),
        BackgroundColor3 = bgCard,
        BackgroundTransparency = 0.5,
        BorderSizePixel = 0,
        LayoutOrder = order,
        Parent = parent,
        Children = {
            New("UICorner", { CornerRadius = UDim.new(0, S(8)) }),
            New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.95, Thickness = 1 }),
        },
    })
    New("TextLabel", {
        Size = UDim2.new(1, -S(76), 1, 0),
        Position = UDim2.new(0, S(14), 0, 0),
        BackgroundTransparency = 1,
        Text = name,
        TextWrapped = true,
        TextXAlignment = Enum.TextXAlignment.Left,
        Font = Enum.Font.GothamSemibold,
        TextSize = S(14),
        TextColor3 = textMain,
        TextTransparency = 0.35,
        Parent = box,
    })
    local switch = New("Frame", {
        Size = UDim2.new(0, S(42), 0, S(24)),
        AnchorPoint = Vector2.new(1, 0.5),
        Position = UDim2.new(1, -S(14), 0.5, 0),
        BackgroundColor3 = bgTrack,
        BorderSizePixel = 0,
        Parent = box,
        Children = {
            New("UICorner", { CornerRadius = UDim.new(1, 0) }),
            New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.9, Thickness = 1 }),
        },
    })
    local knob = New("Frame", {
        Size = UDim2.new(0, S(16), 0, S(16)),
        AnchorPoint = Vector2.new(0, 0.5),
        Position = UDim2.new(0, S(3), 0.5, 0),
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BackgroundTransparency = 0.3,
        BorderSizePixel = 0,
        Parent = switch,
        Children = { New("UICorner", { CornerRadius = UDim.new(1, 0) }) },
    })
    local btn = New("TextButton", {
        Size = UDim2.new(1, 0, 1, 0),
        BackgroundTransparency = 1,
        Text = "",
        AutoButtonColor = false,
        ZIndex = 5,
        Parent = box,
    })
    local function Update()
        if state then
            TweenService:Create(switch, TweenInfo.new(0.15), { BackgroundColor3 = cyan }):Play()
            TweenService:Create(knob, TweenInfo.new(0.15), { Position = UDim2.new(0, S(42) - S(16) - S(3), 0.5, 0) }):Play()
            TweenService:Create(knob, TweenInfo.new(0.15), { BackgroundTransparency = 0 }):Play()
        else
            TweenService:Create(switch, TweenInfo.new(0.15), { BackgroundColor3 = bgTrack }):Play()
            TweenService:Create(knob, TweenInfo.new(0.15), { Position = UDim2.new(0, S(3), 0.5, 0) }):Play()
            TweenService:Create(knob, TweenInfo.new(0.15), { BackgroundTransparency = 0.3 }):Play()
        end
    end
    btn.MouseButton1Click:Connect(function()
        state = not state
        Update()
        pcall(callback, state)
    end)
    Update()
    return {
        Set = function(v) state = v; Update() end,
        Get = function() return state end,
    }
end

local function CreateSlider(parent, name, min, max, current, suffix, callback)
    local order = parent.NextOrder()
    local box = New("Frame", {
        Size = UDim2.new(1, -S(30), 0, S(60)),
        BackgroundColor3 = bgCard,
        BackgroundTransparency = 0.5,
        BorderSizePixel = 0,
        LayoutOrder = order,
        Parent = parent,
        Children = {
            New("UICorner", { CornerRadius = UDim.new(0, S(8)) }),
            New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.95, Thickness = 1 }),
        },
    })
    New("TextLabel", {
        Size = UDim2.new(1, -S(70), 0, S(20)),
        Position = UDim2.new(0, S(14), 0, S(8)),
        BackgroundTransparency = 1,
        Text = name,
        TextXAlignment = Enum.TextXAlignment.Left,
        Font = Enum.Font.GothamSemibold,
        TextSize = S(13),
        TextColor3 = textMain,
        TextTransparency = 0.35,
        Parent = box,
    })
    local valLabel = New("TextLabel", {
        Size = UDim2.new(0, S(60), 0, S(20)),
        Position = UDim2.new(1, -S(74), 0, S(8)),
        BackgroundTransparency = 1,
        Text = tostring(current) .. suffix,
        Font = Enum.Font.GothamBold,
        TextSize = S(12),
        TextColor3 = cyan,
        TextXAlignment = Enum.TextXAlignment.Right,
        Parent = box,
    })
    local track = New("Frame", {
        Size = UDim2.new(1, -S(28), 0, S(6)),
        Position = UDim2.new(0, S(14), 0, S(38)),
        BackgroundColor3 = bgTrack,
        BorderSizePixel = 0,
        Parent = box,
        Children = { New("UICorner", { CornerRadius = UDim.new(1, 0) }) },
    })
    local fill = New("Frame", {
        Size = UDim2.new(0, 0, 1, 0),
        BackgroundColor3 = cyan,
        BorderSizePixel = 0,
        Parent = track,
        Children = { New("UICorner", { CornerRadius = UDim.new(1, 0) }) },
    })
    local knob = New("TextButton", {
        Size = UDim2.new(0, S(18), 0, S(18)),
        AnchorPoint = Vector2.new(0.5, 0.5),
        Position = UDim2.new(0, 0, 0.5, 0),
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BorderSizePixel = 0,
        Text = "",
        AutoButtonColor = false,
        Parent = track,
        Children = {
            New("UICorner", { CornerRadius = UDim.new(1, 0) }),
            New("UIStroke", { Color = cyan, Thickness = 1, Transparency = 0.4 }),
        },
    })
    local dragging = false
    local function SetFromX(x)
        local trackW = track.AbsoluteSize.X
        if trackW <= 0 then return end
        local rel = (x - track.AbsolutePosition.X) / trackW
        rel = Clamp(rel, 0, 1)
        local v = min + (max - min) * rel
        v = math.floor(v + 0.5)
        fill.Size = UDim2.new(rel, 0, 1, 0)
        knob.Position = UDim2.new(rel, 0, 0.5, 0)
        valLabel.Text = tostring(v) .. suffix
        pcall(callback, v)
    end
    knob.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            SetFromX(input.Position.X)
        end
    end)
    knob.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = false
        end
    end)
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            SetFromX(input.Position.X)
        end
    end)
    local initRel = Clamp((current - min) / (max - min), 0, 1)
    fill.Size = UDim2.new(initRel, 0, 1, 0)
    knob.Position = UDim2.new(initRel, 0, 0.5, 0)
    return { Set = function(v) valLabel.Text = tostring(v) .. suffix; fill.Size = UDim2.new(Clamp((v - min) / (max - min), 0, 1), 0, 1, 0); knob.Position = UDim2.new(Clamp((v - min) / (max - min), 0, 1), 0, 0.5, 0) end }
end

local function CreateButton(parent, text, callback, color)
    local order = parent.NextOrder()
    local btn = New("TextButton", {
        Size = UDim2.new(1, -S(30), 0, S(40)),
        BackgroundColor3 = color or Color3.fromRGB(20, 20, 20),
        Text = text,
        TextWrapped = true,
        TextColor3 = Color3.fromRGB(255, 255, 255),
        TextTransparency = 0.2,
        Font = Enum.Font.GothamBold,
        TextSize = S(13),
        AutoButtonColor = false,
        BorderSizePixel = 0,
        LayoutOrder = order,
        Parent = parent,
        Children = {
            New("UICorner", { CornerRadius = UDim.new(0, S(8)) }),
            New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.95, Thickness = 1 }),
        },
    })
    btn.MouseButton1Click:Connect(function()
        pcall(callback, btn)
    end)
    return btn
end

local function CreateTextBox(parent, name, defaultValue, callback)
    local order = parent.NextOrder()
    local box = New("Frame", {
        Size = UDim2.new(1, -S(30), 0, S(40)),
        BackgroundColor3 = bgCard,
        BackgroundTransparency = 0.5,
        BorderSizePixel = 0,
        LayoutOrder = order,
        Parent = parent,
        Children = {
            New("UICorner", { CornerRadius = UDim.new(0, S(8)) }),
            New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.95, Thickness = 1 }),
        },
    })
    New("TextLabel", {
        Size = UDim2.new(0, S(110), 1, 0),
        Position = UDim2.new(0, S(14), 0, 0),
        BackgroundTransparency = 1,
        Text = name,
        TextXAlignment = Enum.TextXAlignment.Left,
        Font = Enum.Font.GothamSemibold,
        TextSize = S(13),
        TextColor3 = textMain,
        TextTransparency = 0.35,
        Parent = box,
    })
    local input = New("TextBox", {
        Size = UDim2.new(1, -S(130), 1, -S(12)),
        Position = UDim2.new(0, S(120), 0, S(6)),
        BackgroundColor3 = bgTrack,
        Text = defaultValue,
        TextColor3 = Color3.fromRGB(255, 255, 255),
        TextTransparency = 0.15,
        Font = Enum.Font.GothamSemibold,
        TextSize = S(13),
        PlaceholderColor3 = Color3.fromRGB(255, 255, 255),
        Parent = box,
        Children = { New("UICorner", { CornerRadius = UDim.new(0, S(6)) }) },
    })
    input.FocusLost:Connect(function(enter)
        if enter then pcall(callback, input.Text) end
    end)
    return input
end

local function CreateDropdown(parent, name, options, currentIndex, callback)
    local order = parent.NextOrder()
    local state = currentIndex or 1
    local box = New("Frame", {
        Size = UDim2.new(1, -S(30), 0, S(40)),
        BackgroundColor3 = bgCard,
        BackgroundTransparency = 0.5,
        BorderSizePixel = 0,
        LayoutOrder = order,
        Parent = parent,
        Children = {
            New("UICorner", { CornerRadius = UDim.new(0, S(8)) }),
            New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.95, Thickness = 1 }),
        },
    })
    New("TextLabel", {
        Size = UDim2.new(0, S(120), 1, 0),
        Position = UDim2.new(0, S(14), 0, 0),
        BackgroundTransparency = 1,
        Text = name,
        TextXAlignment = Enum.TextXAlignment.Left,
        Font = Enum.Font.GothamSemibold,
        TextSize = S(13),
        TextColor3 = textMain,
        TextTransparency = 0.35,
        Parent = box,
    })
    local label = New("TextLabel", {
        Size = UDim2.new(1, -S(150), 1, 0),
        Position = UDim2.new(0, S(120), 0, 0),
        BackgroundTransparency = 1,
        Text = options[state],
        TextWrapped = true,
        Font = Enum.Font.GothamBold,
        TextSize = S(13),
        TextColor3 = cyan,
        TextXAlignment = Enum.TextXAlignment.Right,
        Parent = box,
    })
    local arrow = New("TextButton", {
        Size = UDim2.new(0, S(36), 1, 0),
        Position = UDim2.new(1, -S(36), 0, 0),
        BackgroundTransparency = 1,
        Text = "▼",
        TextColor3 = Color3.fromRGB(255, 255, 255),
        TextTransparency = 0.3,
        Font = Enum.Font.GothamBold,
        TextSize = S(13),
        AutoButtonColor = false,
        Parent = box,
    })
    local list = New("ScrollingFrame", {
        Size = UDim2.new(1, -S(4), 0, math.min(#options * (S(30) + S(2)) + S(8), S(160))),
        Position = UDim2.new(0, S(2), 0, S(40)),
        BackgroundColor3 = Color3.fromRGB(8, 8, 8),
        BackgroundTransparency = 0.05,
        BorderSizePixel = 0,
        Visible = false,
        ZIndex = 8,
        ScrollBarThickness = S(3),
        AutomaticCanvasSize = Enum.AutomaticSize.Y,
        Parent = box,
    })
    New("UIPadding", {
        PaddingTop = UDim.new(0, S(4)),
        PaddingBottom = UDim.new(0, S(4)),
    }, list)
    New("UIListLayout", {
        Padding = UDim.new(0, S(2)),
        SortOrder = Enum.SortOrder.LayoutOrder,
    }, list)
    for i, opt in ipairs(options) do
        local b = New("TextButton", {
            Size = UDim2.new(1, -S(8), 0, S(30)),
            Position = UDim2.new(0, S(4), 0, 0),
            BackgroundColor3 = Color3.fromRGB(20, 20, 20),
            Text = opt,
            TextColor3 = Color3.fromRGB(255, 255, 255),
            TextTransparency = 0.25,
            Font = Enum.Font.GothamSemibold,
            TextSize = S(13),
            AutoButtonColor = false,
            ZIndex = 9,
            Parent = list,
            Children = { New("UICorner", { CornerRadius = UDim.new(0, S(6)) }) },
        })
        b.MouseButton1Click:Connect(function()
            state = i
            label.Text = opt
            list.Visible = false
            pcall(callback, opt)
        end)
    end
    arrow.MouseButton1Click:Connect(function()
        list.Visible = not list.Visible
    end)
    return {
        Set = function(i) state = i; label.Text = options[i] end,
        Get = function() return state end,
    }
end

-- ==========================================================
-- VARIABLES DE ESTADO DEL JUGADOR
-- ==========================================================
local justDied = false
local attackHeight = 40
local selectedWeaponType = "Sword"

local autoDungeonActive = false
local currentTarget = nil
local lastTarget = nil

localPlayer.CharacterAdded:Connect(function(newCharacter)
    character = newCharacter
    humanoidRootPart = newCharacter:WaitForChild("HumanoidRootPart", 10)
    justDied = true
end)

local netFolder = nil
local registerAttack = nil
local registerHit = nil
local commF = nil
local enemiesFolder = Workspace:FindFirstChild("Enemies") or Workspace

T.spawn(function()
    pcall(function()
        netFolder = ReplicatedStorage:WaitForChild("Modules", 15):WaitForChild("Net", 15)
        registerAttack = netFolder:WaitForChild("RE/RegisterAttack", 5)
        registerHit = netFolder:WaitForChild("RE/RegisterHit", 5)
        commF = ReplicatedStorage:WaitForChild("Remotes", 15):WaitForChild("CommF_", 15)
    end)
end)

-- ==========================================================
-- COMBAT: FAST ATTACK NORMAL
-- ==========================================================
local FlaggedFastAttRange = 1200
local COOLDOWN = 0.01
local FlaggedEnabled = false
local FlaggedThread = nil

local function GetNearbyTargets(hrp, range)
    local targets = {}
    for _, folder in ipairs({ Workspace:FindFirstChild("Enemies"), Workspace:FindFirstChild("Characters") }) do
        if folder then
            for _, v in ipairs(folder:GetChildren()) do
                local vHRP = v:FindFirstChild("HumanoidRootPart")
                local vHum = v:FindFirstChild("Humanoid")
                if v ~= character and vHRP and vHum and vHum.Health > 0 and (vHRP.Position - hrp.Position).Magnitude <= range then
                    for _, part in ipairs(v:GetChildren()) do
                        if part:IsA("BasePart") then targets[#targets + 1] = { v, part } end
                    end
                end
            end
        end
    end
    return targets
end

local function DoFlaggedFastAttack()
    local _Char = localPlayer.Character
    if not _Char then return end
    local _HRP = _Char:FindFirstChild("HumanoidRootPart")
    if not _HRP then return end
    local _Tool = _Char:FindFirstChildOfClass("Tool")
    if not _Tool then return end
    local wt = _Tool:GetAttribute("WeaponType")
    if wt ~= "Melee" and wt ~= "Sword" then return end
    local _targets = GetNearbyTargets(_HRP, FlaggedFastAttRange)
    if #_targets > 0 then
        pcall(function()
            require(ReplicatedStorage.Modules.Net):RemoteEvent("RegisterHit", true)
            ReplicatedStorage.Modules.Net["RE/RegisterAttack"]:FireServer()
            local _Head = _targets[1][1]:FindFirstChild("Head")
            if _Head then
                ReplicatedStorage.Modules.Net["RE/RegisterHit"]:FireServer(_Head, _targets)
            end
        end)
    end
end

local function StartFlagged()
    if FlaggedThread then T.cancel(FlaggedThread) end
    FlaggedThread = T.spawn(function()
        while FlaggedEnabled do
            T.wait(COOLDOWN)
            pcall(DoFlaggedFastAttack)
        end
    end)
end

local function StopFlagged()
    if FlaggedThread then
        T.cancel(FlaggedThread)
        FlaggedThread = nil
    end
end

local faToggle = nil

local function ToggleFastAttack()
    FlaggedEnabled = not FlaggedEnabled
    if FlaggedEnabled then
        StartFlagged()
    else
        StopFlagged()
    end
    if faToggle then faToggle.Set(FlaggedEnabled) end
end

-- ==========================================================
-- COMBAT: FAST ATTACK AC FLAGGED (Bypass)
-- ==========================================================
local u4_flagged = nil
local u5_flagged = 0
pcall(function()
    u4_flagged = ReplicatedStorage:FindFirstChild("u4_flagged")
    local u5 = ReplicatedStorage:FindFirstChild("u5_flagged")
    if u5 then u5_flagged = u5.Value or 0 end
end)

local FlaggedBypassEnabled = false
local FlaggedBypassThread = nil

local function DoFlaggedBypassAttack()
    local _Char = localPlayer.Character
    if not _Char then return end
    local _HRP = _Char:FindFirstChild("HumanoidRootPart")
    if not _HRP then return end
    local _Tool = _Char:FindFirstChildOfClass("Tool")
    if not _Tool then return end
    local wt = _Tool:GetAttribute("WeaponType")
    if wt ~= "Melee" and wt ~= "Sword" then return end
    local _targets = GetNearbyTargets(_HRP, FlaggedFastAttRange)
    if #_targets > 0 then
        pcall(function()
            require(ReplicatedStorage.Modules.Net):RemoteEvent("RegisterHit", true)
            ReplicatedStorage.Modules.Net["RE/RegisterAttack"]:FireServer()
            local _Head = _targets[1][1]:FindFirstChild("Head")
            if _Head then
                ReplicatedStorage.Modules.Net["RE/RegisterHit"]:FireServer(_Head, _targets, {}, tostring(localPlayer.UserId):sub(2, 4) .. tostring(coroutine.running()):sub(11, 15))
                if u4_flagged and Bxor then
                    Cloneref(u4_flagged):FireServer(
                        string.gsub("RE/RegisterHit", ".", function(c) return string.char(Bxor(string.byte(c), math.floor(Workspace:GetServerTimeNow() / 10 % 10) + 1)) end),
                        Bxor(u5_flagged + 909090, ReplicatedStorage.Modules.Net.seed:InvokeServer() * 2), _Head, _targets)
                end
            end
        end)
    end
end

local function StartFlaggedBypass()
    if FlaggedBypassThread then T.cancel(FlaggedBypassThread) end
    FlaggedBypassThread = T.spawn(function()
        while FlaggedBypassEnabled do
            T.wait(COOLDOWN)
            pcall(DoFlaggedBypassAttack)
        end
    end)
end

local function StopFlaggedBypass()
    if FlaggedBypassThread then
        T.cancel(FlaggedBypassThread)
        FlaggedBypassThread = nil
    end
end

local faBypassToggle = nil

local function ToggleFlaggedBypass()
    FlaggedBypassEnabled = not FlaggedBypassEnabled
    if FlaggedBypassEnabled then
        StartFlaggedBypass()
    else
        StopFlaggedBypass()
    end
    if faBypassToggle then faBypassToggle.Set(FlaggedBypassEnabled) end
end

-- ==========================================================
-- COMBAT: AUTO V4
-- ==========================================================
local v4Enabled = false

local function ActivarV4()
    pcall(function()
        local char = localPlayer.Character
        if char and char:FindFirstChild("Awakening") then
            char.Awakening.RemoteFunction:InvokeServer(true)
        elseif localPlayer.Backpack and localPlayer.Backpack:FindFirstChild("Awakening") then
            localPlayer.Backpack.Awakening.RemoteFunction:InvokeServer(true)
        end
    end)
end

T.spawn(function()
    while true do
        T.wait(1)
        if v4Enabled then pcall(ActivarV4) end
    end
end)

-- ==========================================================
-- FARMEO: DUNGEON COMPLETO
-- ==========================================================
local function IsHakiActive()
    if not character then return false end
    if character:FindFirstChild("HasBuso") then return true end
    return false
end

local function ForceActivateHaki()
    if not character or not autoDungeonActive then return end
    if not IsHakiActive() then
        pcall(function()
            if commF then commF:InvokeServer("Buso") end
        end)
    end
end

local function ForceEquipWeapon()
    if not character or not autoDungeonActive then return end
    local currentTool = character:FindFirstChildOfClass("Tool")
    if currentTool then
        local tt = currentTool.ToolTip or ""
        if selectedWeaponType == "Sword" and (tt == "Sword" or string.find(string.lower(currentTool.Name), "sword") or string.find(string.lower(currentTool.Name), "katana")) then
            return
        elseif selectedWeaponType == "Melee" and tt == "Melee" then
            return
        end
    end
    local backpack = localPlayer:FindFirstChildOfClass("Backpack")
    if backpack then
        for _, tool in ipairs(backpack:GetChildren()) do
            if tool:IsA("Tool") then
                local match = false
                local tt = tool.ToolTip or ""
                if selectedWeaponType == "Sword" and (tt == "Sword" or string.find(string.lower(tool.Name), "sword") or string.find(string.lower(tool.Name), "katana")) then
                    match = true
                elseif selectedWeaponType == "Melee" and tt == "Melee" then
                    match = true
                end
                if match then
                    local humanoid = character:FindFirstChildOfClass("Humanoid")
                    if humanoid then
                        humanoid:EquipTool(tool)
                        break
                    end
                end
            end
        end
    end
end

local function StartAutoBuffs()
    T.spawn(function()
        while autoDungeonActive do
            pcall(ForceEquipWeapon)
            T.wait(0.3)
        end
    end)
    T.spawn(function()
        while autoDungeonActive do
            pcall(ForceActivateHaki)
            T.wait(0.1)
        end
    end)
end

local function GetClosestEnemy()
    if justDied then return nil end
    local closestEnemy = nil
    local shortestDistance = math.huge
    local currentCharacter = localPlayer.Character
    if not currentCharacter or not currentCharacter:FindFirstChild("HumanoidRootPart") then return nil end
    local myPos = currentCharacter.HumanoidRootPart.Position
    local sourceFolder = Workspace:FindFirstChild("Enemies") or enemiesFolder
    if sourceFolder then
        local isBossAlive = false
        local gasKnight = sourceFolder:FindFirstChild("Gas Knight")
        if gasKnight then
            local bossHum = gasKnight:FindFirstChildOfClass("Humanoid")
            if bossHum and bossHum.Health > 0 then isBossAlive = true end
        end
        local children = sourceFolder:GetChildren()
        local primaryCandidates = {}
        local backupCandidates = {}
        for i = 1, #children do
            local object = children[i]
            if not (object.Name == "PropHitboxPlaceholder" and not isBossAlive) then
                if object:IsA("Model") and object ~= currentCharacter and object.Name ~= "Blank Buddy" and object.Name ~= "Sombra" and object.Name ~= "Shadow" then
                    local humanoid = object:FindFirstChildOfClass("Humanoid")
                    local root = object:FindFirstChild("HumanoidRootPart") or object:FindFirstChild("Head")
                    if humanoid and humanoid.Health > 0 and root and object.Parent then
                        local distance = (root.Position - myPos).Magnitude
                        if object ~= lastTarget then
                            table.insert(primaryCandidates, { model = object, dist = distance })
                        else
                            table.insert(backupCandidates, { model = object, dist = distance })
                        end
                    end
                end
            end
        end
        local targetList = #primaryCandidates > 0 and primaryCandidates or backupCandidates
        for i = 1, #targetList do
            local candidate = targetList[i]
            if candidate.dist < shortestDistance then
                shortestDistance = candidate.dist
                closestEnemy = candidate.model
            end
        end
    end
    return closestEnemy
end

local function TeleportToEnemy(enemy)
    if not enemy or not enemy.Parent or not humanoidRootPart then return false end
    local enemyRoot = enemy:FindFirstChild("HumanoidRootPart") or enemy:FindFirstChild("Head")
    if not enemyRoot then return false end
    humanoidRootPart.CFrame = enemyRoot.CFrame * CFrame.new(0, attackHeight, 0)
    return true
end

local function AttackEnemy(enemy)
    if not enemy then return end
    pcall(function()
        local _targets = {}
        for _, part in ipairs(enemy:GetChildren()) do
            if part:IsA("BasePart") then _targets[#_targets + 1] = { enemy, part } end
        end
        if #_targets == 0 then return end
        local _Head = enemy:FindFirstChild("Head")
        if registerAttack then registerAttack:FireServer() end
        if registerHit and _Head then
            registerHit:FireServer(_Head, _targets)
        end
    end)
end

local function GetCurrentRoomIndex(rooms)
    if not humanoidRootPart then return 1 end
    local myPos = humanoidRootPart.Position
    local currentRoomIdx = 1
    local minDistanceToRoomCenter = math.huge
    for i, room in ipairs(rooms) do
        local roomPart = room:FindFirstChildOfClass("Part") or room:FindFirstChildOfClass("MeshPart") or room:FindFirstChild("Floor")
        if roomPart then
            local dist = (roomPart.Position - myPos).Magnitude
            if dist < minDistanceToRoomCenter then
                minDistanceToRoomCenter = dist
                currentRoomIdx = i
            end
        end
    end
    return currentRoomIdx
end

local function AutoPassRoom()
    local mapFolder = Workspace:FindFirstChild("Map")
    if not mapFolder then return false end
    local dungeonFolder = mapFolder:FindFirstChild("Dungeon")
    if not dungeonFolder then return false end
    local rooms = dungeonFolder:GetChildren()
    if #rooms == 0 then return false end
    table.sort(rooms, function(a, b)
        local numA = tonumber(string.match(a.Name, "%d+")) or 0
        local numB = tonumber(string.match(b.Name, "%d+")) or 0
        return numA < numB
    end)
    local currentRoomIdx = GetCurrentRoomIndex(rooms)
    local activeRoom = rooms[currentRoomIdx]
    if activeRoom then
        local teleporter = activeRoom:FindFirstChild("ExitTeleporter")
        if teleporter then
            local rootPart = teleporter:FindFirstChild("Root")
            if rootPart and rootPart:IsA("BasePart") and humanoidRootPart then
                local touchTransmitter = rootPart:FindFirstChildOfClass("TouchTransmitter")
                if touchTransmitter then
                    humanoidRootPart.CFrame = rootPart.CFrame
                    T.wait(0.12)
                    if firetouchinterest then
                        firetouchinterest(humanoidRootPart, rootPart, 0)
                        T.wait(0.1)
                        firetouchinterest(humanoidRootPart, rootPart, 1)
                    end
                    if justDied then justDied = false end
                    return true
                end
            end
        end
    end
    return false
end

local function RotateTargets()
    while autoDungeonActive do
        currentTarget = GetClosestEnemy()
        if currentTarget and not justDied then
            lastTarget = currentTarget
            local enemyHumanoid = currentTarget:FindFirstChildOfClass("Humanoid")
            local timer = 0
            while autoDungeonActive and currentTarget and currentTarget.Parent and enemyHumanoid and enemyHumanoid.Health > 0 and timer < 0.35 and not justDied do
                pcall(TeleportToEnemy, currentTarget)
                pcall(AttackEnemy, currentTarget)
                T.wait(0.01)
                timer = timer + 0.01
            end
        else
            if justDied then
                local passed = false
                local attempts = 0
                while autoDungeonActive and justDied and attempts < 10 do
                    passed = AutoPassRoom()
                    if passed then
                        justDied = false
                        break
                    end
                    attempts = attempts + 1
                    T.wait(0.5)
                end
                if attempts >= 10 then justDied = false end
            else
                T.wait(1.0)
                local passed = AutoPassRoom()
                if not passed then
                    T.wait(0.5)
                end
            end
        end
    end
end

-- ==========================================================
-- FARMEO: AUTO V4 POR TECLA (Y) + AUTO CARTAS
-- ==========================================================
local autoV4Active = false
T.spawn(function()
    while true do
        T.wait(0.5)
        if autoV4Active and autoDungeonActive and character then
            pcall(function()
                VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.Y, false, game)
                T.wait(0.1)
                VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.Y, false, game)
            end)
        end
    end
end)

local autoBlessingActive = false
local PriorityList = { "Sword Damage", "Fortress", "Life Steal", "Race Meter", "Defense", "Health", "Energy" }

T.spawn(function()
    while true do
        if autoBlessingActive then
            pcall(function()
                local possiblePaths = {
                    PlayerGui:FindFirstChild("BlessingGui"),
                    PlayerGui:FindFirstChild("ChooseBlessing"),
                    PlayerGui:FindFirstChild("Main") and PlayerGui.Main:FindFirstChild("ChooseBlessing"),
                }
                for _, gui in ipairs(possiblePaths) do
                    if gui and gui.Enabled then
                        local container = gui:FindFirstChild("Buttons") or gui:FindFirstChild("Container") or gui:FindFirstChild("Frame")
                        if container then
                            for _, priorityName in ipairs(PriorityList) do
                                local prioridadLimpia = string.gsub(string.upper(priorityName), " ", "")
                                for _, button in ipairs(container:GetChildren()) do
                                    if button:IsA("TextButton") or button:IsA("ImageButton") or button:IsA("Frame") then
                                        local textoCarta = button.Name
                                        for _, desc in ipairs(button:GetDescendants()) do
                                            if desc:IsA("TextLabel") then
                                                textoCarta = textoCarta .. desc.Text
                                            end
                                        end
                                        local textLimpio = string.gsub(string.upper(textoCarta), " ", "")
                                        if string.find(textLimpio, prioridadLimpia) then
                                            pcall(function()
                                                local clickTarget = button
                                                if button:IsA("Frame") then
                                                    clickTarget = button:FindFirstChildOfClass("TextButton") or button
                                                end
                                                if clickTarget.MouseButton1Click then clickTarget.MouseButton1Click:Fire() end
                                                if clickTarget:IsA("TextButton") or clickTarget:IsA("ImageButton") then
                                                    if clickTarget.Activated then clickTarget.Activated:Fire() end
                                                end
                                            end)
                                            T.wait(2)
                                        end
                                    end
                                end
                            end
                        end
                    end
                end
            end)
        end
        T.wait(0.3)
    end
end)

-- ==========================================================
-- SISTEMA BOAT FLY
-- ==========================================================
local isFlying = false
local isAuto = false
local isDancing = false
local isTweening = false
local defaultSpeed = 150
local targetHeight = 50
local currentDance = "Vaca"

local lv, ao, attachment, renderConnection, trackerConnection
local activeTween = nil
local lastBoatSeat = nil

local findBoatBtn = nil
local boatManualToggle = nil
local boatAutoToggle = nil
local boatDanceToggle = nil
local speedBox = nil
local heightBox = nil

local function SetBoatToggles(mode)
    if boatManualToggle then boatManualToggle.Set(mode == "Manual") end
    if boatAutoToggle then boatAutoToggle.Set(mode == "Auto") end
    if boatDanceToggle then boatDanceToggle.Set(mode == "Dance") end
end

local trackerGui = Instance.new("BillboardGui")
trackerGui.Name = "EdsuBoatTracker"
trackerGui.AlwaysOnTop = true
trackerGui.Size = UDim2.new(0, 150, 0, 50)
trackerGui.StudsOffset = Vector3.new(0, 8, 0)

local trackerText = Instance.new("TextLabel")
trackerText.Size = UDim2.new(1, 0, 1, 0)
trackerText.BackgroundTransparency = 1
trackerText.TextColor3 = Color3.fromRGB(255, 255, 255)
trackerText.TextStrokeTransparency = 0.5
trackerText.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
trackerText.TextScaled = true
trackerText.Font = Enum.Font.GothamBlack
trackerText.Parent = trackerGui

local function monitorSeating(char)
    local humanoid = char:WaitForChild("Humanoid", 5)
    if humanoid then
        humanoid:GetPropertyChangedSignal("SeatPart"):Connect(function()
            if humanoid.SeatPart then
                lastBoatSeat = humanoid.SeatPart
                trackerGui.Adornee = lastBoatSeat
                if not trackerGui.Parent then trackerGui.Parent = CoreGui end
            end
        end)
    end
end

trackerConnection = RunService.Heartbeat:Connect(function()
    pcall(function()
        if lastBoatSeat and lastBoatSeat.Parent and localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
            local myPos = localPlayer.Character.HumanoidRootPart.Position
            local boatPos = lastBoatSeat.Position
            local dist = (boatPos - myPos).Magnitude
            trackerText.Text = string.format("Tu Barco\n[%d m]", math.floor(dist))
            trackerGui.Enabled = true
        else
            trackerGui.Enabled = false
        end
    end)
end)

if localPlayer.Character then monitorSeating(localPlayer.Character) end
localPlayer.CharacterAdded:Connect(monitorSeating)

local function GetBestBoatSeat()
    local boatsFolder = Workspace:FindFirstChild("Boats")
    if boatsFolder then
        for _, boat in pairs(boatsFolder:GetChildren()) do
            local owner = boat:FindFirstChild("Owner")
            if owner and tostring(owner.Value) == tostring(localPlayer.Name) then
                local seat = boat:FindFirstChild("VehicleSeat") or boat:FindFirstChildWhichIsA("VehicleSeat", true)
                if seat then
                    lastBoatSeat = seat
                    trackerGui.Adornee = lastBoatSeat
                    if not trackerGui.Parent then trackerGui.Parent = CoreGui end
                    return seat
                end
            end
        end
    end
    return lastBoatSeat
end

local function stopFlying()
    isFlying = false
    isAuto = false
    isDancing = false
    SetBoatToggles(nil)
    if renderConnection then renderConnection:Disconnect() renderConnection = nil end
    if lv then lv:Destroy() lv = nil end
    if ao then ao:Destroy() ao = nil end
    if attachment then attachment:Destroy() attachment = nil end
end

local function TweenToBoat(callback)
    local char = localPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    local humanoid = char and char:FindFirstChild("Humanoid")
    if not hrp or not humanoid then return end

    if isTweening then
        if activeTween then activeTween:Cancel() end
        isTweening = false
        hrp.Anchored = false
        if findBoatBtn then
            findBoatBtn.Text = "🔍 BUSCAR MI BARCO"
            findBoatBtn.BackgroundColor3 = Color3.fromRGB(30, 20, 0)
        end
        return
    end

    local bestSeat = GetBestBoatSeat()
    if not bestSeat or not bestSeat.Parent then
        if findBoatBtn then
            findBoatBtn.Text = "❌ SIN BARCO"
            T.wait(1)
            findBoatBtn.Text = "🔍 BUSCAR MI BARCO"
        end
        return
    end

    isTweening = true
    if findBoatBtn then
        findBoatBtn.Text = "🛑 CANCELAR VIAJE"
        findBoatBtn.BackgroundColor3 = Color3.fromRGB(30, 0, 0)
    end

    local dist = (bestSeat.Position - hrp.Position).Magnitude
    local tweenTime = dist / 250

    hrp.Anchored = true
    local tweenInfo = TweenInfo.new(tweenTime, Enum.EasingStyle.Linear)
    activeTween = TweenService:Create(hrp, tweenInfo, { CFrame = bestSeat.CFrame })
    activeTween:Play()

    local connection
    connection = activeTween.Completed:Connect(function(playbackState)
        connection:Disconnect()
        activeTween = nil
        if not isTweening then return end
        isTweening = false
        if playbackState == Enum.PlaybackState.Completed then
            hrp.Anchored = false
            bestSeat:Sit(humanoid)
            T.wait(0.2)
            if findBoatBtn then
                findBoatBtn.Text = "🔍 BUSCAR MI BARCO"
                findBoatBtn.BackgroundColor3 = Color3.fromRGB(30, 20, 0)
            end
            if callback then callback() end
        end
    end)
end

local function startFlying(mode)
    if isTweening then return end
    local char = localPlayer.Character
    if not char then return end
    local humanoid = char:FindFirstChild("Humanoid")
    if not humanoid then return end

    if not humanoid.SeatPart then
        TweenToBoat(function() startFlying(mode) end)
        return
    end

    local seat = humanoid.SeatPart
    stopFlying()

    attachment = Instance.new("Attachment")
    attachment.Parent = seat

    lv = Instance.new("LinearVelocity")
    lv.Attachment0 = attachment
    lv.MaxForce = math.huge
    lv.VelocityConstraintMode = Enum.VelocityConstraintMode.Vector
    lv.Parent = seat

    ao = Instance.new("AlignOrientation")
    ao.Attachment0 = attachment
    ao.Mode = Enum.OrientationAlignmentMode.OneAttachment
    ao.MaxTorque = math.huge
    ao.Responsiveness = 200
    ao.Parent = seat

    isFlying = true
    isAuto = (mode == "Auto")
    isDancing = (mode == "Dance")
    SetBoatToggles(mode)

    renderConnection = RunService.Heartbeat:Connect(function()
        pcall(function()
            if not char or not humanoid or not humanoid.SeatPart or humanoid.SeatPart ~= seat then
                stopFlying()
                return
            end
            local currentY = seat.Position.Y
            local currentSpeed = speedBox and tonumber(speedBox.Text) or defaultSpeed
            local currentTargetHeight = heightBox and tonumber(heightBox.Text) or targetHeight
            local cam = Workspace.CurrentCamera
            if not cam then return end

            local lookVec = cam.CFrame.LookVector
            local forwardDir = Vector3.new(lookVec.X, 0, lookVec.Z).Unit
            local baseCFrame = CFrame.lookAt(Vector3.zero, forwardDir)
            local timeVar = tick()

            if isDancing then
                if currentDance == "Vaca" then
                    lv.VectorVelocity = Vector3.new(0, math.sin(timeVar * 12) * 25, 0)
                    ao.CFrame = baseCFrame * CFrame.Angles(math.rad(40) + (math.sin(timeVar * 12) * 0.15), math.sin(timeVar * 6) * 1.2, math.cos(timeVar * 12) * 0.3)
                elseif currentDance == "Borracho" then
                    lv.VectorVelocity = Vector3.new(math.sin(timeVar * 1.5) * 20, math.sin(timeVar * 2) * 15, math.cos(timeVar * 1.2) * 20)
                    ao.CFrame = baseCFrame * CFrame.Angles(math.sin(timeVar * 3) * 0.4, math.sin(timeVar * 1.5) * 2, math.cos(timeVar * 2) * 0.4)
                elseif currentDance == "Si" then
                    lv.VectorVelocity = Vector3.new(0, 0, 0)
                    ao.CFrame = baseCFrame * CFrame.Angles(math.sin(timeVar * 6) * 0.6, 0, 0)
                elseif currentDance == "No" then
                    lv.VectorVelocity = Vector3.new(0, 0, 0)
                    ao.CFrame = baseCFrame * CFrame.Angles(0, math.sin(timeVar * 7) * 0.7, 0)
                elseif currentDance == "Feliz" then
                    lv.VectorVelocity = Vector3.new(0, math.abs(math.sin(timeVar * 10)) * 30, 0)
                    ao.CFrame = baseCFrame * CFrame.Angles(math.sin(timeVar * 15) * 0.1, 0, 0)
                end
            elseif isAuto then
                local yVelocity = (currentTargetHeight - currentY) * 3
                lv.VectorVelocity = Vector3.new(forwardDir.X * currentSpeed, yVelocity, forwardDir.Z * currentSpeed)
                ao.CFrame = baseCFrame
            else
                local moveDir = humanoid.MoveDirection
                local yVelocity = (currentTargetHeight - currentY) * 3
                lv.VectorVelocity = Vector3.new(moveDir.X * currentSpeed, yVelocity, moveDir.Z * currentSpeed)
                ao.CFrame = baseCFrame
            end
        end)
    end)
end

-- ==========================================================
-- SISTEMAS: AIMBOT / KILL AURA / HITBOX
-- ==========================================================
local eds_AimbotEnabled = false
local eds_AimbotRange = 1200
local eds_AimbotSmooth = 0.3

local function eds_EnemyFolders()
    local folders = {}
    local e = Workspace:FindFirstChild("Enemies")
    if e then folders[#folders + 1] = e end
    local c = Workspace:FindFirstChild("Characters")
    if c then folders[#folders + 1] = c end
    return folders
end

local function eds_IsEnemy(v)
    if v == character then return false end
    if v:IsA("Model") and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChildOfClass("Humanoid") then
        return true
    end
    return false
end

local function eds_GetEnemies()
    local list = {}
    for _, folder in ipairs(eds_EnemyFolders()) do
        for _, v in ipairs(folder:GetChildren()) do
            if eds_IsEnemy(v) then list[#list + 1] = v end
        end
    end
    return list
end

local function eds_ClosestEnemy(maxDist)
    if not humanoidRootPart then return nil end
    local best = nil
    local bestD = maxDist or 1e9
    for _, v in ipairs(eds_GetEnemies()) do
        local hrp = v:FindFirstChild("HumanoidRootPart")
        if hrp then
            local d = (hrp.Position - humanoidRootPart.Position).Magnitude
            if d < bestD then best, bestD = v, d end
        end
    end
    return best
end

RunService.RenderStepped:Connect(function(dt)
    if not eds_AimbotEnabled then return end
    if not UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then return end
    pcall(function()
        local target = eds_ClosestEnemy(eds_AimbotRange)
        if not target then return end
        local hrp = target:FindFirstChild("HumanoidRootPart")
        local cam = Workspace.CurrentCamera
        if not (hrp and cam) then return end
        local pos = hrp.Position + Vector3.new(0, hrp.Size.Y * 0.35, 0)
        local _, onScreen = cam:WorldToScreenPoint(pos)
        if onScreen then
            local look = CFrame.lookAt(cam.CFrame.Position, pos)
            local alpha = Clamp(eds_AimbotSmooth * 60 * dt, 0, 1)
            cam.CFrame = cam.CFrame:Lerp(look, alpha)
        end
    end)
end)

local eds_KillAuraEnabled = false
local eds_KillAuraRange = 25
local eds_KillAuraConnection = nil

local function eds_StopKillAura()
    if eds_KillAuraConnection then
        eds_KillAuraConnection:Disconnect()
        eds_KillAuraConnection = nil
    end
end

local function eds_StartKillAura()
    eds_StopKillAura()
    eds_KillAuraConnection = RunService.Heartbeat:Connect(function()
        if not eds_KillAuraEnabled then return end
        pcall(function()
            if not humanoidRootPart then return end
            local origin = humanoidRootPart.Position
            for _, v in ipairs(eds_GetEnemies()) do
                local hrp = v:FindFirstChild("HumanoidRootPart")
                local hum = v:FindFirstChildOfClass("Humanoid")
                if hrp and hum and hum.Health > 0 and (hrp.Position - origin).Magnitude <= eds_KillAuraRange then
                    hum:TakeDamage(hum.MaxHealth * 2)
                end
            end
        end)
    end)
end

local eds_HitboxEnabled = false
local eds_HitboxSize = 4

local function eds_ApplyHitboxes(scaleVal)
    for _, v in ipairs(eds_GetEnemies()) do
        local hrp = v:FindFirstChild("HumanoidRootPart")
        if hrp and not hrp:GetAttribute("EdsOriginalSize") then
            hrp:SetAttribute("EdsOriginalSize", hrp.Size)
        end
    end
    for _, v in ipairs(eds_GetEnemies()) do
        local hrp = v:FindFirstChild("HumanoidRootPart")
        if hrp then
            local base = hrp:GetAttribute("EdsOriginalSize") or hrp.Size
            if scaleVal then
                local maxSide = math.max(base.X, base.Y, base.Z)
                hrp.Size = Vector3.new(maxSide * scaleVal, maxSide * scaleVal, maxSide * scaleVal)
            else
                hrp.Size = base
            end
        end
    end
end

T.spawn(function()
    while true do
        T.wait(0.5)
        if eds_HitboxEnabled then pcall(eds_ApplyHitboxes, eds_HitboxSize) end
    end
end)

-- ==========================================================
-- SISTEMA FARM 1-2800 + QUEST
-- ==========================================================
local eds_FarmEnabled = false
local eds_FarmTargetLevel = 2800
local eds_FarmUseHaki = true
local eds_FarmStatsBuy = true
local eds_ChestEnabled = false
local eds_FarmThread = nil
local eds_StatusThread = nil
local eds_FarmStatus = "Inactivo"
local eds_MoneyEarned = 0
local eds_Kills = 0
local eds_PrevQuestText = ""
local eds_statusLabel = nil

local function fq(sea, level, npcName, questId, enemyName, cframe, sell)
    return { sea = sea, level = level, npcName = npcName, questId = questId, enemyName = enemyName, pos = cframe, sell = sell }
end

local FarmQuests = {
    fq(1, 5, "BanditQuestGiver", "BanditQuest1", "Bandit", CFrame.new(1062.3, 16.4, 1546.6), 100),
    fq(1, 10, "PirateQuestGiver", "PirateQuest1", "Pirate", CFrame.new(1114.8, 20.1, 1406.2), 250),
    fq(1, 15, "GunnerQuestGiver", "GunnerQuest1", "Gunner", CFrame.new(1040.5, 19.5, 1240.3), 500),
    fq(1, 20, "SwanQuestGiver", "SwanQuest1", "Swan", CFrame.new(1065.2, 15.1, 912.7), 1000),
    fq(1, 30, "MarineCaptainQuestGiver", "MarineCaptainQuest1", "Marine Captain", CFrame.new(1263.4, 13.2, 790.1), 2000),
    fq(1, 40, "FishmanQuestGiver", "FishmanQuest1", "Fishman Warrior", CFrame.new(1234.6, 5.0, 650.8), 4000),
    fq(1, 50, "FishmanCommanderQuestGiver", "FishmanCommanderQuest1", "Fishman Commando", CFrame.new(1260.3, 5.0, 550.4), 8000),
    fq(1, 60, "CursedGunnerQuestGiver", "CursedGunnerQuest1", "Cursed Gunner", CFrame.new(1150.2, 6.4, 480.6), 15000),
    fq(1, 70, "AssassinQuestGiver", "AssassinQuest1", "Assassin", CFrame.new(1020.8, 6.5, 420.3), 30000),
    fq(1, 80, "RoyalSquadQuestGiver", "RoyalSquadQuest1", "Royal Squad", CFrame.new(980.6, 12.5, 350.2), 50000),
    fq(1, 90, "BlackKnightQuestGiver", "BlackKnightQuest1", "Black Knight", CFrame.new(1080.4, 13.0, 280.5), 75000),
    fq(1, 100, "SkyQuestGiver", "SkyQuest1", "Sky", CFrame.new(1150.2, 25.0, 200.4), 100000),
    fq(1, 110, "SpookyQuestGiver", "SpookyQuest1", "Spooky", CFrame.new(1180.6, 13.0, 150.3), 150000),
    fq(1, 120, "PirateAdmiralQuestGiver", "PirateAdmiralQuest1", "Pirate Admiral", CFrame.new(1220.5, 12.0, 100.6), 200000),
    fq(1, 130, "AssassinAdmiralQuestGiver", "AssassinAdmiralQuest1", "Assassin Admiral", CFrame.new(1260.3, 11.0, 50.8), 300000),
    fq(1, 140, "PirateLordQuestGiver", "PirateLordQuest1", "Pirate Lord", CFrame.new(1300.7, 10.0, 0.5), 500000),
    fq(1, 150, "GalleyCaptainQuestGiver", "GalleyCaptainQuest1", "Galley Captain", CFrame.new(1340.2, 9.5, -40.3), 750000),
    fq(2, 300, "CyborgQuestGiver", "CyborgQuest1", "Cyborg", CFrame.new(-1506.4, 17.5, 882.2), 1000000),
    fq(2, 375, "CyborgV2QuestGiver", "CyborgV2Quest1", "Cyborg V2", CFrame.new(-1482.1, 14.0, 928.4), 2500000),
    fq(2, 450, "SkyGodQuestGiver", "SkyGodQuest1", "Sky God", CFrame.new(-1460.5, 20.0, 750.3), 5000000),
    fq(2, 525, "AngelQuestGiver", "AngelQuest1", "Angel", CFrame.new(-1245.3, 18.0, 835.6), 10000000),
    fq(2, 600, "CyborgV3QuestGiver", "CyborgV3Quest1", "Cyborg V3", CFrame.new(-1210.2, 13.0, 720.8), 25000000),
    fq(2, 675, "MermanQuestGiver", "MermanQuest1", "Merman", CFrame.new(-1048.6, 12.0, 610.4), 50000000),
    fq(2, 750, "DragonQuestGiver", "DragonQuest1", "Dragon", CFrame.new(-915.4, 8.0, 515.2), 100000000),
    fq(2, 825, "PirateKingQuestGiver", "PirateKingQuest1", "Pirate King", CFrame.new(-780.3, 7.0, 455.7), 150000000),
    fq(2, 900, "MarineEmperorQuestGiver", "MarineEmperorQuest1", "Marine Emperor", CFrame.new(-660.8, 6.0, 430.5), 200000000),
    fq(2, 975, "DemonQuestGiver", "DemonQuest1", "Demon", CFrame.new(-550.2, 5.0, 400.3), 250000000),
    fq(3, 1100, "DarkMarineQuestGiver", "DarkMarineQuest1", "Dark Marine", CFrame.new(4410.2, 5.0, 2250.4), 300000000),
    fq(3, 1300, "PrisonerQuestGiver", "PrisonerQuest1", "Prisoner", CFrame.new(4300.5, 5.0, 2100.8), 350000000),
    fq(3, 1500, "CursedDuelistQuestGiver", "CursedDuelistQuest1", "Cursed Duelist", CFrame.new(4200.3, 6.0, 1950.6), 400000000),
    fq(3, 1700, "WarlordQuestGiver", "WarlordQuest1", "Warlord", CFrame.new(4100.8, 7.0, 1800.2), 450000000),
    fq(3, 1900, "ShogunQuestGiver", "ShogunQuest1", "Shogun", CFrame.new(4000.2, 8.0, 1650.7), 500000000),
    fq(3, 2100, "EliteHunterQuestGiver", "EliteHunterQuest1", "Elite Hunter", CFrame.new(3900.6, 9.0, 1500.3), 550000000),
    fq(3, 2300, "DemonHunterQuestGiver", "DemonHunterQuest1", "Demon Hunter", CFrame.new(3800.4, 10.0, 1350.6), 600000000),
    fq(3, 2500, "SeaBeastQuestGiver", "SeaBeastQuest1", "Sea Beast", CFrame.new(3700.8, 11.0, 1200.2), 650000000),
    fq(3, 2800, "LeviathanQuestGiver", "LeviathanQuest1", "Leviathan", CFrame.new(3600.2, 12.0, 1050.8), 1000000000),
}

local function eds_PlayerLevel()
    local data = localPlayer:FindFirstChild("Data")
    if data and data:FindFirstChild("Level") then return data.Level.Value or 0 end
    return 0
end

local function eds_PlayerBeli()
    local data = localPlayer:FindFirstChild("Data")
    if data and data:FindFirstChild("Beli") then return data.Beli.Value or 0 end
    return 0
end

local function eds_CurrentQuest()
    local gui = localPlayer:FindFirstChild("PlayerGui")
    if not gui then return "" end
    local main = gui:FindFirstChild("Main")
    if not main then return "" end
    local quest = main:FindFirstChild("Quest")
    if not quest then return "" end
    local container = quest:FindFirstChild("Container")
    local title = container and container:FindFirstChild("QuestTitle")
    local label = title and title:FindFirstChild("Title")
    if label then return tostring(label.Text) end
    return ""
end

local function eds_CurrentQuestEnemy()
    local txt = tostring(eds_CurrentQuest())
    if txt == "" then return nil end
    local name = txt:match("Defeat%s+(%d+)%s+(.+)")
    if not name then name = txt:match("Defeat%s+(.+)") end
    return name
end

local function eds_FindEnemyByName(name)
    if not name then return nil end
    local best = nil
    local bestD = 1e9
    local origin = humanoidRootPart and humanoidRootPart.Position or Vector3.new()
    for _, v in ipairs(eds_GetEnemies()) do
        if v.Name == name then
            local hrp = v:FindFirstChild("HumanoidRootPart")
            if hrp then
                local d = (hrp.Position - origin).Magnitude
                if d < bestD then best, bestD = v, d end
            end
        end
    end
    return best
end

local function eds_CurrentSea()
    local sea1 = Workspace:FindFirstChild("Sea1")
    local sea2 = Workspace:FindFirstChild("Sea2")
    local sea3 = Workspace:FindFirstChild("Sea3")
    local pos = humanoidRootPart and humanoidRootPart.Position or Vector3.new()
    local d1 = sea1 and (sea1.Position - pos).Magnitude or math.huge
    local d2 = sea2 and (sea2.Position - pos).Magnitude or math.huge
    local d3 = sea3 and (sea3.Position - pos).Magnitude or math.huge
    if d2 <= d1 and d2 <= d3 then return 2 end
    if d3 <= d1 and d3 <= d2 then return 3 end
    return 1
end

local function eds_TargetQuest()
    local lvl = eds_PlayerLevel()
    local best = nil
    for _, q in ipairs(FarmQuests) do
        if lvl >= q.level then best = q end
    end
    return best
end

local function eds_SeaTown()
    local sea = eds_CurrentSea()
    if sea == 2 then return CFrame.new(-1138.2, 12.3, 884.8) end
    if sea == 3 then return CFrame.new(4410.2, 5.0, 2250.4) end
    return CFrame.new(1055.0, 16.0, 1522.0)
end

local function eds_TeleportTo(pos)
    if humanoidRootPart and pos then
        humanoidRootPart.CFrame = CFrame.new(pos.X, pos.Y, pos.Z)
    end
end

local function eds_FindNPC(npcName)
    if not npcName then return nil end
    local npcs = Workspace:FindFirstChild("NPCs")
    if not npcs then return nil end
    for _, npc in ipairs(npcs:GetChildren()) do
        if npc.Name == npcName then
            local hrp = npc:FindFirstChild("HumanoidRootPart") or npc:FindFirstChild("RootPart") or npc.PrimaryPart
            if hrp then return hrp end
        end
    end
    return nil
end

local function eds_AcceptQuest(q)
    local ok = false
    if commF then
        ok = pcall(function()
            commF:InvokeServer("AcceptQuest", q.questId, 1)
        end)
    end
    return ok
end

local function eds_EquipBestWeapon()
    local containers = { localPlayer.Backpack, localPlayer.Character }
    local best = nil
    local bestDmg = 0
    local first = nil
    for _, c in ipairs(containers) do
        if c then
            for _, t in ipairs(c:GetChildren()) do
                if t:IsA("Tool") then
                    if not first then first = t end
                    local dmg = 0
                    if t:FindFirstChild("Damage") then
                        local dv = t.Damage.Value or t.Damage
                        dmg = tonumber(dv) or 0
                    end
                    if dmg > bestDmg then best, bestDmg = t, dmg end
                end
            end
        end
    end
    if not best then best = first end
    if best and localPlayer.Character and best.Parent ~= localPlayer.Character then
        best.Parent = localPlayer.Character
    end
end

local function eds_UseHaki()
    pcall(function()
        local toolName = "God's Guard"
        local char = localPlayer.Character
        if char and char:FindFirstChild(toolName) then
            char[toolName].Parent = localPlayer.Backpack
        end
        local bp = localPlayer.Backpack
        if bp and bp:FindFirstChild(toolName) then
            bp[toolName].Parent = localPlayer.Character
        end
    end)
end

local function eds_BuyStats()
    if not eds_FarmStatsBuy then return end
    pcall(function()
        local beli = eds_PlayerBeli()
        local count = math.floor(beli / 5000)
        count = math.min(count, 100)
        if count <= 0 then return end
        if commF then
            for _ = 1, count do
                commF:InvokeServer("AddStats", "Defense", 1)
            end
        end
    end)
end

local function eds_AttackEnemy(enemy)
    pcall(function()
        local char = localPlayer.Character
        if not char then return end
        local tool = char:FindFirstChildOfClass("Tool")
        if not tool then return end
        local targets = {}
        for _, part in ipairs(enemy:GetChildren()) do
            if part:IsA("BasePart") then targets[#targets + 1] = { enemy, part } end
        end
        if #targets == 0 then return end
        local head = enemy:FindFirstChild("Head") or (targets[1][1]:FindFirstChild("Head"))
        local net = require(ReplicatedStorage.Modules.Net)
        net["RE/RegisterAttack"]:FireServer()
        if head then
            net["RE/RegisterHit"]:FireServer(head, targets)
        end
    end)
end

local function eds_FarmIteration()
    local char = localPlayer.Character
    if not char then return true end
    local hrp = char:FindFirstChild("HumanoidRootPart")
    if not hrp then return true end

    local prevText = tostring(eds_PrevQuestText)
    local currentText = tostring(eds_CurrentQuest())
    if prevText ~= "" and currentText == "" then
        local completed = eds_TargetQuest()
        if completed then
            eds_MoneyEarned = eds_MoneyEarned + (completed.sell or 0)
            eds_Kills = eds_Kills + 5
        end
    end
    eds_PrevQuestText = currentText

    local lvl = eds_PlayerLevel()
    if lvl >= eds_FarmTargetLevel then
        eds_FarmStatus = "🏆 Nivel objetivo alcanzado! (" .. lvl .. ")"
        return false
    end

    local q = eds_TargetQuest()
    if not q then
        eds_FarmStatus = "❓ Sin quest para nivel " .. lvl
        return true
    end

    local sea = eds_CurrentSea()
    if q.sea ~= sea then
        eds_FarmStatus = "🌊 Viajando al Sea " .. q.sea
        local targetSea = q.sea == 2 and Workspace:FindFirstChild("Sea2") or (q.sea == 3 and Workspace:FindFirstChild("Sea3") or Workspace:FindFirstChild("Sea1"))
        if targetSea then
            eds_TeleportTo(targetSea.Position + Vector3.new(0, 100, 0))
        else
            eds_TeleportTo(eds_SeaTown())
        end
        return true
    end

    eds_EquipBestWeapon()
    if eds_FarmUseHaki then eds_UseHaki() end
    eds_BuyStats()

    local questEnemy = eds_CurrentQuestEnemy()
    if not questEnemy then
        local npc = eds_FindNPC(q.npcName)
        if npc then
            eds_TeleportTo(npc.Position + Vector3.new(0, 5, 0))
            T.wait(0.3)
            local ok = eds_AcceptQuest(q)
            if ok then
                eds_FarmStatus = "📜 Quest aceptada: " .. tostring(q.enemyName)
            else
                eds_FarmStatus = "⚠️ No se pudo aceptar la quest: " .. tostring(q.enemyName)
            end
        else
            eds_TeleportTo(eds_SeaTown())
            eds_FarmStatus = "🗺️ Buscando NPC: " .. tostring(q.npcName)
        end
        return true
    end

    local enemy = eds_FindEnemyByName(questEnemy)
    if not enemy then
        eds_TeleportTo(q.pos)
        eds_FarmStatus = "⏳ Esperando enemigos: " .. tostring(questEnemy) .. " (lvl " .. lvl .. ")"
        return true
    end

    local ehrp = enemy:FindFirstChild("HumanoidRootPart")
    if ehrp then
        local dist = (ehrp.Position - hrp.Position).Magnitude
        if dist > 12 then
            local dir = (ehrp.Position - hrp.Position).Unit
            eds_TeleportTo(ehrp.Position - dir * 10)
        else
            local tool = char:FindFirstChildOfClass("Tool")
            if tool then
                eds_AttackEnemy(enemy)
            end
        end
    end
    eds_FarmStatus = "⚔️ Farmeando: " .. tostring(questEnemy) .. " | Nivel: " .. lvl
    return true
end

local function eds_FarmLoop()
    while eds_FarmEnabled do
        local ok, err = pcall(eds_FarmIteration)
        if not ok then
            eds_FarmStatus = "⚠️ Error: " .. tostring(err)
        end
        T.wait(0.3)
    end
end

local function eds_StartFarm()
    eds_StopFarm()
    if isFlying then stopFlying() end
    eds_FarmEnabled = true
    eds_FarmThread = T.spawn(eds_FarmLoop)
    if eds_StatusThread then T.cancel(eds_StatusThread) end
    eds_StatusThread = T.spawn(function()
        while eds_FarmEnabled do
            pcall(function()
                if eds_statusLabel then
                    eds_statusLabel.Text = "📊 " .. eds_FarmStatus .. " | 💰 $" .. tostring(eds_MoneyEarned) .. " | 💀 " .. tostring(eds_Kills)
                end
            end)
            T.wait(0.5)
        end
    end)
end

local function eds_StopFarm()
    eds_FarmEnabled = false
    if eds_FarmThread then
        T.cancel(eds_FarmThread)
        eds_FarmThread = nil
    end
    pcall(function()
        if eds_statusLabel then
            eds_statusLabel.Text = "📊 " .. eds_FarmStatus .. " | 💰 $" .. tostring(eds_MoneyEarned)
        end
    end)
end

T.spawn(function()
    while true do
        T.wait(1)
        if eds_ChestEnabled then
            pcall(function()
                local chest = Workspace:FindFirstChild("Chest")
                if chest and humanoidRootPart then
                    local hrp2 = chest:FindFirstChild("HumanoidRootPart") or chest.PrimaryPart
                    if hrp2 then
                        humanoidRootPart.CFrame = hrp2.CFrame + Vector3.new(0, 5, 0)
                    end
                end
            end)
        end
    end
end)

-- ==========================================================
-- SISTEMA PLAYERS
-- ==========================================================
local eds_PlayersEnabled = false
local eds_PlayerListParent = nil

local function eds_RefreshPlayerList()
    if not eds_PlayerListParent then return end
    for _, child in ipairs(eds_PlayerListParent:GetChildren()) do
        if child:IsA("TextButton") then child:Destroy() end
    end
    local sorted = {}
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= localPlayer then sorted[#sorted + 1] = plr end
    end
    table.sort(sorted, function(a, b)
        local lvlA = 0
        local lvlB = 0
        if a:FindFirstChild("Data") and a.Data:FindFirstChild("Level") then lvlA = a.Data.Level.Value or 0 end
        if b:FindFirstChild("Data") and b.Data:FindFirstChild("Level") then lvlB = b.Data.Level.Value or 0 end
        return lvlA > lvlB
    end)
    for _, plr in ipairs(sorted) do
        local lvl = 0
        if plr:FindFirstChild("Data") and plr.Data:FindFirstChild("Level") then lvl = plr.Data.Level.Value or 0 end
        local dist = 0
        local tchar = plr.Character
        local thrp = tchar and tchar:FindFirstChild("HumanoidRootPart")
        if thrp and humanoidRootPart then
            dist = (thrp.Position - humanoidRootPart.Position).Magnitude
        end
        local btn = New("TextButton", {
            Size = UDim2.new(1, -S(8), 0, S(34)),
            BackgroundColor3 = Color3.fromRGB(20, 20, 20),
            Text = plr.Name .. "  |  Nv " .. lvl .. "  |  📍 " .. math.floor(dist) .. "m",
            TextColor3 = Color3.fromRGB(255, 255, 255),
            Font = Enum.Font.GothamSemibold,
            TextSize = S(13),
            AutoButtonColor = false,
            Parent = eds_PlayerListParent,
            Children = { New("UICorner", { CornerRadius = UDim.new(0, S(6)) }) },
        })
        btn.MouseButton1Click:Connect(function()
            pcall(function()
                local targetChar = plr.Character
                local targetHRP = targetChar and targetChar:FindFirstChild("HumanoidRootPart")
                if targetHRP then eds_TeleportTo(targetHRP.Position) end
            end)
        end)
    end
end

local function eds_StartPlayers()
    eds_PlayersEnabled = true
    T.spawn(function()
        while eds_PlayersEnabled do
            pcall(eds_RefreshPlayerList)
            T.wait(2)
        end
    end)
end

local function eds_StopPlayers()
    eds_PlayersEnabled = false
end

-- ==========================================================
-- CONSTRUCCION DEL PANEL
-- ==========================================================
local function BuildPanel()
    pcall(function()
        if ScreenGui then ScreenGui:Destroy() end
        ScreenGui = nil
        MainFrame = nil
        Sidebar = nil
        ContentScroll = nil
        HeaderLabel = nil
        TabButtons = {}
        TabFrames = {}
        Headers = {}
    end)

    SetStatus("🔄 Cargando panel (1/4)...")

    local ok, err = pcall(function()
        ScreenGui = Instance.new("ScreenGui")
        ScreenGui.Name = "EdsusitoHubPanel"
        ScreenGui.ResetOnSpawn = false
        ScreenGui.IgnoreGuiInset = true
        ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
        ScreenGui.Parent = ResolveGuiParent()

        MainFrame = New("Frame", {
            Size = UDim2.new(0, S(780), 0, S(520)),
            Position = UDim2.new(0.5, -S(390), 0.5, -S(260)),
            AnchorPoint = Vector2.new(0.5, 0.5),
            BackgroundColor3 = Color3.fromRGB(0, 0, 0),
            BorderSizePixel = 0,
            ClipsDescendants = true,
            Parent = ScreenGui,
            Children = {
                New("UICorner", { CornerRadius = UDim.new(0, S(12)) }),
                New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.95, Thickness = 1 }),
            },
        })

        CreateBgImage(MainFrame)

        Sidebar = New("Frame", {
            Size = UDim2.new(0, S(190), 1, 0),
            BackgroundTransparency = 1,
            BorderSizePixel = 0,
            ZIndex = 3,
            Parent = MainFrame,
            Children = {
                New("Frame", {
                    Size = UDim2.new(0, 1, 1, 0),
                    Position = UDim2.new(1, -1, 0, 0),
                    BackgroundColor3 = Color3.fromRGB(255, 255, 255),
                    BackgroundTransparency = 0.95,
                    BorderSizePixel = 0,
                }),
            },
        })

        New("TextLabel", {
            Size = UDim2.new(1, 0, 0, S(30)),
            Position = UDim2.new(0, 0, 0, S(22)),
            BackgroundTransparency = 1,
            RichText = true,
            Text = 'EDSUSITO<font color="#00e5ff">.</font>',
            Font = Enum.Font.GothamBlack,
            TextSize = S(24),
            TextColor3 = Color3.fromRGB(255, 255, 255),
            TextTransparency = 0.15,
            Parent = Sidebar,
        })
        New("TextLabel", {
            Size = UDim2.new(1, 0, 0, S(16)),
            Position = UDim2.new(0, 0, 0, S(52)),
            BackgroundTransparency = 1,
            Text = "HUB V2.0 PANEL",
            Font = Enum.Font.GothamBold,
            TextSize = S(11),
            TextColor3 = Color3.fromRGB(255, 255, 255),
            TextTransparency = 0.7,
            Parent = Sidebar,
        })

        ContentArea = New("Frame", {
            Size = UDim2.new(1, -S(190), 1, 0),
            Position = UDim2.new(0, S(190), 0, 0),
            BackgroundTransparency = 1,
            ZIndex = 3,
            Parent = MainFrame,
        })

        HeaderLabel = New("TextLabel", {
            Size = UDim2.new(1, -S(48), 0, S(38)),
            Position = UDim2.new(0, S(24), 0, S(18)),
            BackgroundTransparency = 1,
            Text = "🏠 Home",
            TextXAlignment = Enum.TextXAlignment.Left,
            Font = Enum.Font.GothamBlack,
            TextSize = S(24),
            TextColor3 = textMain,
            TextTransparency = 0.15,
            Parent = ContentArea,
        })

        ContentScroll = New("ScrollingFrame", {
            Size = UDim2.new(1, 0, 1, -S(72)),
            Position = UDim2.new(0, 0, 0, S(72)),
            BackgroundTransparency = 1,
            BorderSizePixel = 0,
            ScrollBarThickness = S(4),
            ScrollBarImageTransparency = 0.7,
            AutomaticCanvasSize = Enum.AutomaticSize.Y,
            Parent = ContentArea,
        })
        New("UIListLayout", {
            Padding = UDim.new(0, S(8)),
            HorizontalAlignment = Enum.HorizontalAlignment.Center,
            SortOrder = Enum.SortOrder.LayoutOrder,
        }, ContentScroll)
    end)
    if not ok then
        SetStatus("❌ Error fase 1: " .. tostring(err), Color3.fromRGB(255, 80, 80))
        return
    end

    SetStatus("🔄 Construyendo pestañas (2/4)...")
    ok, err = pcall(function()
        local homeTab = CreateTabFrame("home", "🏠 Home")
        local combatTab = CreateTabFrame("combat", "Combat Features")
        local farmTab = CreateTabFrame("farmeo", "Farmeo Features")
        local settingsTab = CreateTabFrame("settings", "Settings")
        local playersTab = CreateTabFrame("players", "👥 Players")

        CreateTabButton("home", "🏠 Home")
        CreateTabButton("combat", "⚔️ Combat")
        CreateTabButton("farmeo", "⛏️ Farmeo")
        CreateTabButton("settings", "⚙️ Settings")
        CreateTabButton("players", "👥 Players")

        -- HOME
        CreateSection(homeTab, "💬 Comunidad")
        CreateParagraph(homeTab, "Servidor de Discord", "Únete al servidor oficial de Edsusito para soporte, actualizaciones del script, noticias y comunidad.")
        CreateButton(homeTab, "📋 COPIAR INVITACIÓN (discord.gg/UafYeFXuXs)", function(btn)
            local link = "https://discord.gg/UafYeFXuXs"
            if HAS_SETCLIPBOARD then pcall(setclipboard, link) end
            btn.Text = "✅ ¡INVITACIÓN COPIADA!"
            T.delay(2, function()
                btn.Text = "📋 COPIAR INVITACIÓN (discord.gg/UafYeFXuXs)"
            end)
        end, Color3.fromRGB(0, 80, 90))
        CreateLabel(homeTab, "💬 https://discord.gg/UafYeFXuXs")
        CreateLabel(homeTab, "Cualquier duda del panel, contacta por el Discord.")
    end)
    if not ok then SetStatus("⚠️ Fase 2 parcial: " .. tostring(err)) end

    SetStatus("🔄 Construyendo Combat + Farmeo (3/4)...")
    ok, err = pcall(function()
        -- COMBAT
        local combatTab = TabFrames["combat"]
        CreateSection(combatTab, "⚡ Ataque")
        faToggle = CreateToggle(combatTab, "⚡ Fast Attack (Normal)", false, function(v)
            FlaggedEnabled = v
            if v then
                StartFlagged()
            else
                StopFlagged()
            end
        end)
        CreateSlider(combatTab, "Rango de Ataque", 1, 2000, FlaggedFastAttRange, "", function(v)
            FlaggedFastAttRange = v
        end)
        CreateSection(combatTab, "🛡️ Fast Attack AC Flagged")
        faBypassToggle = CreateToggle(combatTab, "🛡️ Fast Attack AC Flagged", false, function(v)
            FlaggedBypassEnabled = v
            if v then
                StartFlaggedBypass()
            else
                StopFlaggedBypass()
            end
        end)
        CreateLabel(combatTab, "🛡️ Solo para ACC marcadas donde el Fast Attack normal no funciona. Actívalo solo si el normal te falla: conlleva riesgo.")
        CreateSection(combatTab, "🔥 V4")
        local v4Toggle = CreateToggle(combatTab, "🔥 Auto V4", false, function(v)
            v4Enabled = v
        end)
        CreateLabel(combatTab, "Teclas: [U] Fast Attack normal · [G] Fast Attack AC Flagged · [N] mostrar/ocultar panel.")
        CreateSection(combatTab, "🎯 Aimbot")
        local aimbotToggle = CreateToggle(combatTab, "🎯 Aimbot (mantén [CTRL])", false, function(v)
            eds_AimbotEnabled = v
        end)
        CreateSlider(combatTab, "Rango Aimbot", 100, 3000, eds_AimbotRange, "", function(v)
            eds_AimbotRange = v
        end)
        CreateSlider(combatTab, "Suavizado Aimbot", 1, 100, eds_AimbotSmooth * 100, "%", function(v)
            eds_AimbotSmooth = v / 100
        end)
        CreateSection(combatTab, "☠️ Kill Aura")
        local killAuraToggle = CreateToggle(combatTab, "☠️ Kill Aura", false, function(v)
            eds_KillAuraEnabled = v
            if v then
                eds_StartKillAura()
            else
                eds_StopKillAura()
            end
        end)
        CreateSlider(combatTab, "Radio Kill Aura", 5, 100, eds_KillAuraRange, " studs", function(v)
            eds_KillAuraRange = v
        end)
        CreateSection(combatTab, "📐 Hitbox Expandido")
        local hitboxToggle = CreateToggle(combatTab, "📐 Hitbox Expandido", false, function(v)
            eds_HitboxEnabled = v
            if not v then
                pcall(eds_ApplyHitboxes, nil)
            end
        end)
        CreateSlider(combatTab, "Tamaño Hitbox", 2, 12, eds_HitboxSize, "", function(v)
            eds_HitboxSize = v
        end)

        -- FARMEO
        local farmTab = TabFrames["farmeo"]
        CreateSection(farmTab, "⚔️ Dungeon")
        local dungeonToggle = CreateToggle(farmTab, "⚔️ Auto Completar Dungeon", false, function(v)
            autoDungeonActive = v
            if v then
                justDied = false
                StartAutoBuffs()
                coroutine.wrap(RotateTargets)()
            else
                currentTarget = nil
                lastTarget = nil
                justDied = false
            end
        end)
        local weaponDropdown = CreateDropdown(farmTab, "🎒 Arma", { "Espada", "Combate (Melee)" }, 1, function(opt)
            if opt == "Espada" then
                selectedWeaponType = "Sword"
            elseif opt == "Combate (Melee)" then
                selectedWeaponType = "Melee"
            end
        end)
        CreateSlider(farmTab, "Altura de Ataque", 10, 100, attackHeight, " studs", function(v)
            attackHeight = v
        end)
        CreateSection(farmTab, "✨ Extras")
        local yv4Toggle = CreateToggle(farmTab, "🎹 Auto V4 (Tecla Y)", false, function(v)
            autoV4Active = v
        end)
        local blessingToggle = CreateToggle(farmTab, "🃏 Auto Elegir Cartas", false, function(v)
            autoBlessingActive = v
        end)
        CreateLabel(farmTab, "🃏 La función de cartas depende de la actualización del juego.")
        CreateSection(farmTab, "⚔️ Farm Automático 1-2800")
        local farmToggle = CreateToggle(farmTab, "⚔️ Auto Farm (Misiones)", false, function(v)
            if v then
                eds_StartFarm()
            else
                eds_StopFarm()
            end
        end)
        CreateSlider(farmTab, "Nivel Objetivo", 100, 2800, eds_FarmTargetLevel, "", function(v)
            eds_FarmTargetLevel = v
        end)
        local farmHakiToggle = CreateToggle(farmTab, "🛡️ Usar Haki", true, function(v)
            eds_FarmUseHaki = v
        end)
        local farmStatsToggle = CreateToggle(farmTab, "📈 Comprar Stats (Defensa)", true, function(v)
            eds_FarmStatsBuy = v
        end)
        eds_statusLabel = CreateLabel(farmTab, "📊 Estado: Inactivo | 💰 $0 | 💀 0")
        local chestToggle = CreateToggle(farmTab, "🎁 Auto Recoger Cofres", false, function(v)
            eds_ChestEnabled = v
        end)
    end)
    if not ok then SetStatus("⚠️ Fase 3 parcial: " .. tostring(err)) end

    SetStatus("🔄 Construyendo Players + Settings (4/4)...")
    ok, err = pcall(function()
        -- PLAYERS
        local playersTab = TabFrames["players"]
        CreateSection(playersTab, "👥 Jugadores")
        local playersList = New("ScrollingFrame", {
            Size = UDim2.new(1, -S(8), 0, 240),
            Position = UDim2.new(0, 4, 0, 0),
            BackgroundColor3 = Color3.fromRGB(10, 10, 10),
            BackgroundTransparency = 0.3,
            BorderSizePixel = 0,
            ScrollBarThickness = S(4),
            AutomaticCanvasSize = Enum.AutomaticSize.Y,
            Parent = playersTab,
        })
        New("UIListLayout", {
            Padding = UDim.new(0, S(4)),
            SortOrder = Enum.SortOrder.LayoutOrder,
        }, playersList)
        New("UIPadding", {
            PaddingTop = UDim.new(0, S(4)),
            PaddingBottom = UDim.new(0, S(4)),
        }, playersList)
        local playersToggle = CreateToggle(playersTab, "👥 Ver Jugadores", false, function(v)
            eds_PlayerListParent = playersList
            if v then
                eds_StartPlayers()
            else
                eds_StopPlayers()
                pcall(eds_RefreshPlayerList)
            end
        end)
        CreateLabel(playersTab, "👥 Haz clic en un jugador para teletransportarte a él. Ordenados por nivel.")

        -- SETTINGS
        local settingsTab = TabFrames["settings"]
        CreateSection(settingsTab, "🚢 Barco")
        findBoatBtn = CreateButton(settingsTab, "🔍 BUSCAR MI BARCO", function()
            TweenToBoat(nil)
        end, Color3.fromRGB(30, 20, 0))
        boatManualToggle = CreateToggle(settingsTab, "🚢 Vuelo Manual", false, function(v)
            if v then
                if not (isFlying and not isAuto and not isDancing) then
                    stopFlying()
                    startFlying("Manual")
                end
            else
                if isFlying and not isAuto and not isDancing then stopFlying() end
            end
        end)
        boatAutoToggle = CreateToggle(settingsTab, "⏩ Auto Avance", false, function(v)
            if v then
                if not (isFlying and isAuto) then
                    stopFlying()
                    startFlying("Auto")
                end
            else
                if isFlying and isAuto then stopFlying() end
            end
        end)
        boatDanceToggle = CreateToggle(settingsTab, "🐄 Modo Baile", false, function(v)
            if v then
                if not (isFlying and isDancing) then
                    stopFlying()
                    startFlying("Dance")
                end
            else
                if isFlying and isDancing then stopFlying() end
            end
        end)
        speedBox = CreateTextBox(settingsTab, "🚀 Velocidad", tostring(defaultSpeed), function() end)
        heightBox = CreateTextBox(settingsTab, "📏 Altura Y", tostring(targetHeight), function() end)
        local danceDropdown = CreateDropdown(settingsTab, "🐄 Baile", { "Vaca Polaca", "Borracho", "Decir que Sí", "Decir que No", "Muy Feliz" }, 1, function(opt)
            local map = { ["Vaca Polaca"] = "Vaca", ["Borracho"] = "Borracho", ["Decir que Sí"] = "Si", ["Decir que No"] = "No", ["Muy Feliz"] = "Feliz" }
            currentDance = map[opt] or "Vaca"
        end)
        CreateSection(settingsTab, "📡 Sistema")
        local webhookToggle = CreateToggle(settingsTab, "📡 Enviar Log a Discord", true, function(v)
            if v then T.spawn(SendLog) end
        end)
        CreateLabel(settingsTab, "Atajos: [N] mostrar/ocultar panel · [U] toggle Fast Attack · [G] toggle AC Flagged.")

        -- BOTONES DE VENTANA
        local MinimizeBtn = New("TextButton", {
            Size = UDim2.new(0, S(26), 0, S(26)),
            Position = UDim2.new(1, -S(66), 0, S(10)),
            BackgroundTransparency = 1,
            Text = "-",
            TextColor3 = Color3.fromRGB(255, 255, 255),
            TextTransparency = 0.4,
            Font = Enum.Font.GothamBold,
            TextSize = S(16),
            AutoButtonColor = false,
            ZIndex = 10,
            Parent = MainFrame,
        })
        local CloseBtn = New("TextButton", {
            Size = UDim2.new(0, S(26), 0, S(26)),
            Position = UDim2.new(1, -S(34), 0, S(10)),
            BackgroundTransparency = 1,
            Text = "✕",
            TextColor3 = Color3.fromRGB(255, 120, 120),
            TextTransparency = 0.25,
            Font = Enum.Font.GothamBold,
            TextSize = S(14),
            AutoButtonColor = false,
            ZIndex = 10,
            Parent = MainFrame,
        })
        MinimizeBtn.MouseButton1Click:Connect(function()
            MainFrame.Visible = false
        end)
        CloseBtn.MouseButton1Click:Connect(function()
            stopFlying()
            if trackerConnection then trackerConnection:Disconnect() end
            if trackerGui then trackerGui:Destroy() end
            if ScreenGui then ScreenGui:Destroy() end
            SetStatus("👋 Panel cerrado. Ejecuta el script de nuevo o pulsa [N] para reabrirlo.")
            T.delay(5, HideStatus)
        end)

        -- ARRASTRAR VENTANA
        local dragging = false
        local dragStart = nil
        local startPos = nil
        local function startDrag(input)
            dragging = true
            dragStart = input.Position
            startPos = MainFrame.Position
        end
        local TitleLabel = Sidebar:FindFirstChildOfClass("TextLabel")
        local SubtitleLabel = Sidebar:FindFirstChildOfClass("TextLabel") and Sidebar:FindFirstChildOfClass("TextLabel")
        local allTitles = {}
        for _, child in ipairs(Sidebar:GetChildren()) do
            if child:IsA("TextLabel") then allTitles[#allTitles + 1] = child end
        end
        for _, lbl in ipairs(allTitles) do
            lbl.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    startDrag(input)
                end
            end)
        end
        UserInputService.InputChanged:Connect(function(input)
            if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                local delta = input.Position - dragStart
                MainFrame.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
            end
        end)
        UserInputService.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                dragging = false
            end
        end)
    end)
    if not ok then SetStatus("⚠️ Fase 4 parcial: " .. tostring(err)) end

    pcall(function()
        if MainFrame then
            MainFrame.Visible = true
            SelectTab("home")
        end
    end)

    SetStatus("✅ Edsusito Hub V2.0 listo. Pulsa [N] si no ves el panel.", Color3.fromRGB(0, 229, 255))
    T.delay(5, HideStatus)
end

-- ==========================================================
-- KEYBINDS GLOBALES
-- ==========================================================
UserInputService.InputBegan:Connect(function(input, gp)
    if gp then return end
    pcall(function()
        if UserInputService:GetFocusedTextBox() then return end
        local key = input.KeyCode
        if key == HUB_CONFIG.KeyTogglePanel then
            if ScreenGui and MainFrame then
                MainFrame.Visible = not MainFrame.Visible
            else
                BuildPanel()
            end
        elseif key == HUB_CONFIG.KeyFastAttack then
            ToggleFastAttack()
        elseif key == HUB_CONFIG.KeyFlaggedBypass then
            ToggleFlaggedBypass()
        end
    end)
end)

-- ==========================================================
-- INICIO
-- ==========================================================
print("✅ EDSUSITO HUB V2.0 cargado correctamente")
SetStatus("🔄 Iniciando Edsusito Hub V2.0...")

T.delay(0.5, function()
    BuildPanel()
    T.spawn(SendLog)
end)
